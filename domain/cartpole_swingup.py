...
class CartPoleSwingUpEnv(gym.Env):
    ...
    def step(self, action, gen, sp_count):
        ...
        if run_type == 'static':
            reward_x = np.cos(((x-self.centre)/self.x_threshold)*(np.pi/2.0))

        # if run_type == 'dynamic':
        #     if np.random.rand() < self.change_point_th:
        #         self.centre += np.random.normal(1, 0.3)
        #
        #         if  self.centre < -self.x_threshold:
        #             while(self.centre > -self.x_threshold):
        #                 self.centre += abs(np.random.normal(0, 0.3))
        #         elif  self.centre > self.x_threshold:
        #             while(self.centre < self.x_threshold):
        #                 self.centre -= abs(np.random.normal(0, 0.3))
        ...
        #         if gen % 7 == 0:
        #             self.change_t += 1
        #             if self.change_t % 2 == 0:
        #                 self.centre = -1.5
        #                 # print('centre {}'.format(self.centre))
        #             elif self.change_t % 2 == 1:
        #                 self.centre = 1.5
        ...
        #         self.centre = -1.5
                        # print('centre {}'.format(self.centre))
            # print('centre : {}'.format(self.centre))
            self.centre_list.append(self.centre)
