...
LENGTH_W = VIEWPORT_W/SCALE
LENGTH_H = VIEWPORT_H/SCALE
...
class LunarLander(gym.Env, EzPickle):
    ...
    def __init__(self):
        ...
        self.reset(gen, sp_count, run_type)
    ...
    def reset(self, gen, sp_count, run_type):
        self._destroy()
        ...
        if run_type == 'static':
            self.helipad_x1 = chunk_x[CHUNKS//2-2]
            self.helipad_x2 = chunk_x[CHUNKS//2+2]
            height[CHUNKS//2-3] = self.helipad_y
            ...
            if (gen >= 100) and (gen < 200):
                self.helipad_x1 = chunk_x[CHUNKS//2-7]
                self.helipad_x2 = chunk_x[CHUNKS//2-3]
                height[CHUNKS//2-8] = self.helipad_y
                height[CHUNKS//2-7] = self.helipad_y
                height[CHUNKS//2-6] = self.helipad_y
                height[CHUNKS//2-5] = self.helipad_y
                height[CHUNKS//2-4] = self.helipad_y
                height[CHUNKS//2-3] = self.helipad_y
                height[CHUNKS//2-2] = self.helipad_y
            elif gen >= 200:
                self.helipad_x1 = chunk_x[CHUNKS//2+3]
                self.helipad_x2 = chunk_x[CHUNKS//2+7]
                height[CHUNKS//2+2] = self.helipad_y
                height[CHUNKS//2+3] = self.helipad_y
                height[CHUNKS//2+4] = self.helipad_y
                height[CHUNKS//2+5] = self.helipad_y
        ...
        if self.continuous:
            return self.step(np.array([0, 0]), gen, sp_count, run_type)[0]
        else:
            return self.step(0, gen, sp_count, run_type)[0]
    ...
    def step(self, action, gen, sp_count, run_type):
        ...
        if run_type == 'static':
            state = [
                (pos.x - LENGTH_W * DEFAULT_CENTRE) / (LENGTH_W * DEFAULT_CENTRE),
                (pos.y - (self.helipad_y+LEG_DOWN/SCALE)) / (LENGTH_H/2),
        ...
        if run_type == 'dynamic':
            if (gen >= 100) and (gen < 200):
                state = [
                    (pos.x - LENGTH_W * 0.25) / (LENGTH_W * DEFAULT_CENTRE),
                    (pos.y - (self.helipad_y+LEG_DOWN/SCALE)) / (LENGTH_H/2),
                    vel.x*(VIEWPORT_W/SCALE/2)/FPS,
                    vel.y*(VIEWPORT_H/SCALE/2)/FPS,
                    self.lander.angle,
                ...
                state = [
                    (pos.x - LENGTH_W * 0.75) / (LENGTH_W * DEFAULT_CENTRE),
                    (pos.y - (self.helipad_y+LEG_DOWN/SCALE)) / (LENGTH_H/2),
                    vel.x*(VIEWPORT_W/SCALE/2)/FPS,
                    vel.y*(VIEWPORT_H/SCALE/2)/FPS,
                    self.lander.angle,
