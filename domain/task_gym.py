...
class GymTask():
  ...
  def getFitness(self, wVec, aVec, gen, sp_count, run_type, hyp=None, view=False, nRep=False, seed=-1):
    ...
    for iRep in range(nRep):
      reward[iRep] = self.testInd(wVec, aVec, view=view, gen=gen, sp_count=sp_count, run_type=run_type, seed=seed+iRep)
    fitness = np.mean(reward)
    return fitness

  def testInd(self, wVec, aVec, view=False, gen=1000, sp_count=1000, run_type='static' ,seed=-1):
    ...
    if seed >= 0:
      ...
      np.random.seed(seed)
      self.env.seed(seed)
    state = self.env.reset(gen, sp_count, run_type)
    self.env.t = 0
    ...
    state, reward, done, info = self.env.step(action, gen, sp_count, run_type)
