...
def assignSpecies(self, species, pop, p):
  ...
  if len(self.species) == 0:
    ...
    self.specnum_max += 1
    species = [Species(pop[0], self.specnum_max)]
    species[0].nOffspring = p['popSize']
    iSpec = 0
  else:
  ...
  for i in range(len(pop)):
    ...
    for iSpec in range(len(species)):
      ...
      if cDist < p['spec_thresh']:
        pop[i].species = species[iSpec].specnum
    ...
    if not assigned:
      self.specnum_max += 1
      pop[i].species = self.specnum_max
      species.append(Species(pop[i], self.specnum_max))
...
def assignOffspring(self, species, pop, p):
  ...
  if nSpecies == 1:
    ...
    specNum = np.asarray([ind_spec.specnum for ind_spec in species])
    ...
    for iSpec in range(nSpecies):
      if not np.any(specId==specNum[iSpec]):
        speciesFit[iSpec] = 0
      else:
        speciesFit[iSpec] = np.mean(rankScore[specId==specNum[iSpec]])
        speciesTop[iSpec] = np.max(popFit[specId==specNum[iSpec]])

        # Did the species improve?
        if speciesTop[iSpec] > species[iSpec].bestFit:
          species[iSpec].bestFit = speciesTop[iSpec]
          bestId = np.argmax(popFit[specId==specNum[iSpec]])
