...
# -- Canonical NEAT recombination operators ------------------------------ -- #
  ...
  def mutAddNode(self, connG, nodeG, innov, gen, p):
    ...
    for i in range(innov.shape[1]):
        if int(connTo[1]) == int(innov[1][i]):
            if int(connTo[2]) == int(innov[2][i]):
                connTo[0] = innov[:,i][0]
                connTo_new = False
                break
    ...
    for i in range(innov.shape[1]):
        if int(connFrom[1]) == int(innov[1][i]):
            if int(connFrom[2]) == int(innov[2][i]):
                connFrom[0] = innov[:,i][0]
                connFrom_new = False
                break
    ...
    if innov is not None:
      if connTo_new == True:
        newInnov = np.empty((5,1))
        newInnov[:,0] = np.hstack((connTo[0:3], newNodeId, gen))
        innov = np.hstack((innov,newInnov))
      if connFrom_new == True:
        newInnov = np.empty((5,1))
        newInnov[:,0] = np.hstack((connFrom[0:3], -1, gen))
        innov = np.hstack((innov,newInnov))
  ...
  def mutAddConn(self, connG, nodeG, innov, gen, p):
    ...
    for src in sources:
      ...
      if len(dest)>0:  # (there is a valid connection)
        ...
        for i in range(innov.shape[1]):
            if int(connNew[1]) == int(innov[1][i]):
                if int(connNew[2]) == int(innov[2][i]):
                    connNew[0] = innov[:,i][0]
                    connNew_new = False
                    # print('{}\n{}'.format(connNew[0:3],innov[:,i][0:3]))
                    break
